...
<!-- PAGE TITLE -->
<h1 class="text-4xl font-extrabold mb-8 text-gray-100">Admin Dashboard</h1>

<!-- WELCOME CARD -->
<div class="bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg max-w-xl mb-10 text-gray-100 border border-white/20">
    <p class="text-lg">
        Welcome, <span class="font-semibold text-yellow-400">
            {{ optional(Auth::user())->full_name ?? optional(Auth::user())->user_id ?? '-' }}
        </span>
    </p>
    <p class="text-sm mt-1">
        Role: <span class="font-semibold text-blue-400">{{ optional(Auth::user())->role ?? '-' }}</span>
    </p>
</div>
            ...
            <div class="flex flex-col space-y-3 mt-2">
                @for ($i = 1; $i <= 5; $i++)
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="counters[]" value="{{ $i }}" class="form-checkbox h-5 w-5 text-blue-500 accent-blue-500 rounded">
        ...
        <div id="counterStatusList" class="flex flex-col gap-2">
            @for ($i = 1; $i <= 5; $i++)
                <div class="flex justify-between items-center p-3 rounded-xl bg-gray-800" id="counterBox{{ $i }}">
                    ...
                    <span class="font-bold text-sm text-white" id="counterStatus{{ $i }}">Checking...</span>
                </div>
            @endfor
        </div>
    </div>
...
<!-- JS for counter form and online status -->
<script>
    ...
    fetch("{{ route('admin.getCounterStatus') }}")
        ...
        .then(data => {
            for(let i = 1; i <= 5; i++) {
                ...
                if(data[i]) {
                    statusEl.textContent = data[i].user + ' - ' + (data[i].status === 'online' ? '🟢 Online' : '🔴 Offline');
                } else {
