...
<div class="flex items-center justify-center min-h-screen bg-blue-900/90 p-4">
    ...
    <div class="w-full max-w-md bg-white/10 backdrop-blur-md rounded-3xl shadow-2xl p-6 md:p-8 border border-white/20 max-h-[95vh] overflow-y-auto">
        ...
        <form method="POST" action="{{ route('admin.updateUser', $user->id) }}" class="space-y-4" autocomplete="off">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="role"
                ...
                <option value="admin" class="text-black" {{ old('role', $user->role)=='admin' ? 'selected' : '' }}>Administrator</option>
                <option value="counter" class="text-black" {{ old('role', $user->role)=='counter' ? 'selected' : '' }}>Counter</option>
        ...
        <div id="counterField">
            ...
            <input type="number"
                name="counter_id"
                id="counter_id"
                value="{{ old('counter_id', $user->counter_id) }}"
